...
| CLASSIFYING | CLASSIFIED_EXPERT_REVIEW | WAITING_EXPERT_CONFIRMATION | 상담 패키지 생성 완료(MVP 자동 차단 항목 미포함) |
...
| PR_CREATING | PR_CREATED | CI_RUNNING | Draft PR 생성 성공 |
...
| WAITING_APPROVAL | APPROVAL_GRANTED | 승인 요청 지점의 다음 상태 | 승인 레코드 존재 (예: 축소 범위 승인 → REQUIREMENT_DRAFTING) |
| WAITING_APPROVAL | APPROVAL_DENIED | CANCELLED 또는 축소 재제안 | 거절 사유 기록 |
| WAITING_EXPERT_CONFIRMATION | EXPERT_ANSWER_UNBLOCKS | 직전 상태 (체크포인트) | 개발자 답변 입력 → 게이트 해제. 답변은 consultation_answers에 기록 |
| WAITING_EXPERT_CONFIRMATION | EXPERT_ANSWER_ADJUSTS_SCOPE | SUPERVISING | 범위 조정 후 재계획 재진입 |
| WAITING_EXPERT_CONFIRMATION | EXPERT_ANSWER_STOPS | CANCELLED | 장기 미응답 시 리마인드 알림 |
...
| FAILED | RETRY_EXHAUSTED_SCOPE_DOWN | SUPERVISING | 사용자 선택: 범위 축소 |
...
| CANCELLED | (터미널) | — | 재시작은 신규 요청/Ticket으로만 |
| TOKEN_BUDGET_EXCEEDED | BUDGET_INCREASE_APPROVED | 직전 상태 (체크포인트) | 사용량·예상 추가량·비용 표시 후 승인. 저성능 모델 자동 전환 금지 |
| TOKEN_BUDGET_EXCEEDED | BUDGET_INCREASE_DENIED | SUPERVISING 또는 CANCELLED | 범위 축소 제안 수락 시 재계획, 거절 시 중단 |
...
| MCP_CONNECTION_FAILED | MCP_ALTERNATIVE_SELECTED | 직전 상태 또는 SUPERVISING | 기존 MCP 대안으로 대체 |
...
| AUTO_BLOCKED_BY_POLICY | ALTERNATIVE_ACCEPTED | REQUIREMENT_DRAFTING | 차단 요소 제외한 축소 요구사항으로 재시작 |
